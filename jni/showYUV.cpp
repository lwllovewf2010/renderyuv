...
static void render(
    ...
    //ALOGE("[%s]%d",__FILE__,__LINE__);
     m_ANativeWindow = nativeWindow;
    int err;
    int mCropWidth = width;
    int mCropHeight = height;

    int halFormat = 0x32315659;//HAL_PIXEL_FORMAT_YV12;//颜色空间
    ...
    CHECK_EQ(0,
            ...
            | GRALLOC_USAGE_HW_TEXTURE | 0x00002000/*GRALLOC_USAGE_EXTERNAL_DISP*/));
    ...
    //申请一块空闲的图形缓冲区
    if ((err = ANativeWindow_lock(m_ANativeWindow, &buf, 0)) != 0) {
    ...
    if ((err = m_ANativeWindow->queueBuffer(m_ANativeWindow, buf,
            -1)) != 0) {
...
nativeSetVideoSurface(JNIEnv *env, jobject thiz, jobject jsurface){
    ...
    /*
    if(android::Surface::isValid(m_ANativeWindow)){
...
nativeShowYUV(JNIEnv *env, jobject thiz,jbyteArray yuvData,jint width,jint height){
    ...
    render(byteBuf,len,m_ANativeWindow,width,height);
}
